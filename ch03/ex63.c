/*
 * Homework Problem 3.63 from CS:APP: This problem will give you a chance to
 * reverse engineer a switch statement from disassembled machine code. In the
 * following procedure, the body of the switch statement has been omitted:
 *
 ...
 *     }
 *
 * The disassembled machine code for the procedure:
 *
 *     long switch_prob(long x, long n)
 *     x in %rdi, n in %rsi
 *     00000000000400590 <switch_prob>:
 ...
 * indirect jump on line 5 that the jump table begins at address 0x4006f8. Using
 * the GDB debugger, we can examine the six 8-byte words of memory comprising
 * the jump table with the command x/6gx 0x4006f8. GDB prints the following:
 *
 *     (gdb) x/6gx 0x4006f8
